...
class UserService {
  ...
  static async signUp(rawData) {
    ...
    if (verifier.verify_code.toString() !== rawData.email_verify.toString()) {
      return {
        response: false,
    ...
    // Check invite code and find inviter user
    if (
      rawData.invite_code !== undefined &&
      rawData.invite_code.trim() !== ""
    ) {
      const inviteCode = parseInt(rawData.invite_code);

      if (!isNaN(inviteCode) && inviteCode !== 0) {
    ...
    const registerData = {
      email: rawData.email,
      password: rawData.password,
      invite_code: rawData.invite_code || null,
